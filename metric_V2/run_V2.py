    ...
    if isinstance(mesh, trimesh.Scene):
        for name, geom in mesh.geometry.items():
            if isinstance(geom, trimesh.Trimesh) and len(geom.vertices) > 0:
                geom = geom.copy()
                geom.vertices = apply_alignment_to_vertices(geom.vertices, align)
...
def main():
    ...
    try:
        ...
        print("Saved metric mesh (.ply):", EXPORT_METRIC_MESH_PATH)
        ...
        if EXPORT_METRIC_MESH_AXIS_ALIGNED_PATH is not None:
            ...
            print("Saved axis-aligned metric mesh (.ply):", EXPORT_METRIC_MESH_AXIS_ALIGNED_PATH)
